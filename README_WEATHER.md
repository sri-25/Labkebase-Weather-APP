...
- **HNSW benchmark, run against live data (167 embedding rows, 20 queries/phase):** WITHOUT index — mean 611ms, median 592ms. WITH index — mean 655ms, median 606ms. The index was not faster here, consistent with the well-known property that HNSW's advantage grows with row count/dimensionality and doesn't necessarily pay off yet at this scale. Also worth noting: `lakebase.py` opens a fresh connection (and re-fetches the Lakebase secret) for every single query — that connection-setup overhead likely dominates these absolute numbers more than the actual vector search does, since both phases pay it equally. A connection pool would give a cleaner read on the index's real effect, but wasn't built here.
